...
import { MARS_LOOK_Y, smoothstep } from './StarAscentScrollRig'
import {
  MARS_ATMOSPHERE,
  MARS_COLOR_GRADE,
  MARS_PLANET,
...
import {
  applyMarsAngularVelocity,
  applyMarsScreenDrag,
  composeMarsOrientation,
  type MarsTrackballVelocity,
} from './marsTrackball'
...
const surfaceFragment = /* glsl */ `
  ...
  void main() {
    vec3 baseColor = pastelize(texture2D(uColorMap, vUv).rgb);
    ...
    float spec = pow(max(dot(normal, halfDir), 0.0), mix(10.0, 32.0, 1.0 - uRoughness));
    spec *= 0.028 * terminator * (1.0 - uRoughness) * shadowMask;

    float night = 1.0 - terminator;
    lit *= mix(1.0, 0.012, night);

    vec3 twilight = baseColor * vec3(0.16, 0.045, 0.028);
    lit = mix(twilight, lit + spec, terminator);
...
type MarsTextures = Awaited<ReturnType<typeof loadMarsTextures>>
...
function MarsAtmosphere({
  ...
  const material = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        uSunDirection: { value: MARS_SUN.direction.clone() },
        uDayColor: { value: MARS_ATMOSPHERE.color },
        uTwilightColor: { value: MARS_ATMOSPHERE.twilight },
        uPower: { value: MARS_ATMOSPHERE.power },
        uIntensity: { value: MARS_ATMOSPHERE.intensity },
        uReveal: { value: 0 },
      },
  ...
  return (
    <mesh scale={MARS_ATMOSPHERE.scale} renderOrder={2}>
...
function MarsSurface({
  ...
  velocityRef: RefObject<MarsTrackballVelocity>
  ...
  const baseTilt = useMemo(
    () =>
      new THREE.Quaternion().setFromEuler(
        new THREE.Euler(MARS_PLANET.tiltX, 0, MARS_PLANET.tiltZ, 'YXZ'),
      ),
    [],
  )
  ...
  const material = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        ...
        uSunDirection: { value: MARS_SUN.direction.clone() },
        uSunIntensity: { value: MARS_SUN.intensity },
        uSunColor: { value: new THREE.Color(MARS_SUN.color) },
        uAmbientColor: { value: new THREE.Color('#030201') },
        uNormalScale: { value: 1.55 },
        uRoughness: { value: 0.95 },
        uDisplacementScale: { value: 0.11 },
        uReveal: { value: 0 },
        uColorSaturation: { value: MARS_COLOR_GRADE.saturation },
        uColorLighten: { value: MARS_COLOR_GRADE.lighten },
      },
  ...
  useFrame((_, delta) => {
    ...
    groupRef.current.position.y = MARS_LOOK_Y
  })

  return (
    <group ref={groupRef} position={[0, MARS_LOOK_Y, 0]}>
      <mesh renderOrder={1}>
        <sphereGeometry
          args={[
            MARS_PLANET.radius,
            MARS_PLANET.widthSegments,
            MARS_PLANET.heightSegments,
          ]}
        />
        <primitive object={material} attach="material" />
      </mesh>
      <MarsAtmosphere radius={MARS_PLANET.radius} revealRef={revealRef} />
    </group>
  )
}
...
export function MarsPlanet({
  ...
  isDraggingRef,
  inertiaDecay,
  rollSpeed = MARS_PLANET.autoSpinSpeed,
  reduceMotion = false,
}: {
  progressRef: RefObject<number>
  orientationRef: RefObject<THREE.Quaternion>
  ...
  const [textures, setTextures] = useState<MarsTextures | null>(null)
