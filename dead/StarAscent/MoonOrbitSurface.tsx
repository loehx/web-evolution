...
import type { MoonTrackballVelocity } from './moonTrackball'

const MOON_ORBIT_MIN_PROGRESS = 0.68
...
export function useMoonTrackballInput(
  ...
  const velocityRef = useRef<MoonTrackballVelocity>({ h: 0, v: 0 })
  ...
  const onPointerDown = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      const progress = progressRef.current ?? 0
      if (progress < MOON_ORBIT_MIN_PROGRESS) return
...
export function MoonOrbitSurface({
  surfaceRef,
  bind,
  isDragging,
  className,
  ...
  surfaceRef: RefObject<HTMLDivElement | null>
  bind: ReturnType<typeof useMoonTrackballInput>['bind']
  isDragging: boolean
  className?: string
}) {
  return (
